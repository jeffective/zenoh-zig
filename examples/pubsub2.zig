...
fn publish() !void {
    ...
    var options = zenoh.Publisher.PutOptions.init();
    try publisher.put(&bytes, &options);

    try std.testing.expectEqualSlices(u8, "key/expression", publisher.keyExpr());
    _ = publisher.id();

    var del_options = zenoh.Publisher.DeleteOptions.init();
    try publisher.delete(&del_options);
}
